...
def get_amp_path(url):
    ...
    try:
        # Replaces the unicode lightning bolt symbol used by amp with the amp
        # keyword
...
def validate_chartbeat_articles():
    ...
    try:
        ...
        send_basic_message(
            'There was an issue reaching the Chartbeat service.' %
            (error), global_store['channel'])
        return 'error'
    ...
    for page in response['pages']:
        page_path = page['path']
        updated_path = page_path.replace('latimes.com', 'sandbox.tronc.arcpublishing.com/pb')
        amp_path = get_amp_path(updated_path)
